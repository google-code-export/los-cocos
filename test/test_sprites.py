...
class SpriteGoto( SpriteLayer ):
    def on_enter( self ):
        self.sprite = ActionSprite( self.image, x=320, y=100, batch=self.batch )

        self.sprite.do( Goto( (620,100), 5 ) )
...
class SpriteRepeatMove( SpriteLayer ):
    def on_enter( self ):
        self.sprite = ActionSprite( self.image, x=120, y=100, batch=self.batch )

        move = Move( (150,0), 0.5 )
        rot = Rotate( 360, 0.5 )

        self.sprite.do( Repeat( Sequence( rot + move , rot , move , rot , move , rot) ) )

class SpriteScale( SpriteLayer ):
    def on_enter( self ):
        self.sprite = ActionSprite( self.image, x=320, y=240, batch=self.batch )

        self.sprite.do( Scale( 10, 5 ) )

class SpriteRotate( SpriteLayer ):
    def on_enter( self ):
        self.sprite = ActionSprite( self.image, x=320, y=100, batch=self.batch )

        self.sprite.do( Rotate( 360, 2 ) )

class SpriteJump( SpriteLayer ):
    def on_enter( self ):
        self.sprite = ActionSprite( self.image, x=120, y=100, batch=self.batch )

        self.sprite.do( Jump(y=100, x=400, jumps=4, duration=3 ) )

class SpriteBezier( SpriteLayer ):
    def on_enter( self ):
        self.sprite = ActionSprite( self.image, x=120, y=100, batch=self.batch )

        self.sprite.do( Bezier( foo.curva, 5 ) )
...
class SpriteDelay( SpriteLayer ):
    def on_enter( self ):
        ...
        self.sprite.do( move + Delay(5) + jump )
...
class SpriteBlink( SpriteLayer ):
    def on_enter( self ):
        ...
        self.sprite.do( blink )

class SpriteFadeOut( SpriteLayer ):
    def on_enter( self ):
        self.sprite1 = ActionSprite( self.image_sister1, x=200, y=240, batch=self.batch )
        self.sprite2 = ActionSprite( self.image_sister2, x=440, y=240, batch=self.batch )

        fadeout = FadeOut( 2 )
        fadein = FadeIn( 2 )

        self.sprite1.opacity = 0
        self.sprite1.do( fadein )
        self.sprite2.do( fadeout )
...
class SpriteRepeatSeq( SpriteLayer ):
    def on_enter( self ):
        ...
        self.sprite.do( Repeat( jump + move + jump2 , 4 ) )
...
class SpriteRepeatSeq2( SpriteLayer ):
    def on_enter( self ):
        ...
        self.sprite.do( Repeat( Repeat(jump,3) + Repeat(move,3) + Repeat(jump2,3) ) )
...
class SpriteTrigger( SpriteLayer ):
    def on_enter( self ):
        ...
        self.sprite.do( move + CallFunc( self.say_hello )  )

    def say_hello( self ):
        print "HELLO BABY"

        self.sprite2 = ActionSprite( self.image_sister1, x=270, y=110, batch=self.batch )


class SpriteReuseAction( SpriteLayer ):
    def on_enter( self ):
        self.sprite1 = ActionSprite( self.image_sister1, x=120, y=250, batch=self.batch )
        self.sprite2 = ActionSprite( self.image_sister2, x=20, y=100, batch=self.batch )

        jump = Jump( 150, 400, 4, 4 )
        self.sprite1.do( jump )
        self.sprite2.do( jump )
...
class SpriteReuseSequence( SpriteLayer ):
    def on_enter( self ):
        ...
        self.sprite1.do( seq )
        self.sprite2.do( seq )
        self.sprite2.do( Repeat( rotate) )


class SpriteAlterTime( SpriteLayer ):
    def on_enter( self ):
        self.sprite1 = ActionSprite( self.image_sister1, x=20, y=100, batch=self.batch )
        self.sprite2 = ActionSprite( self.image_sister2, x=20, y=300, batch=self.batch )

        move1 = Move( (500,0), 3 )
        move2 = Move( (500,0), 3, time_func=accelerate)

        self.sprite1.do( move1 )
        self.sprite2.do( move2 )
